...
import com.cqut.edu.basic.Move;
import com.cqut.edu.basic.Judge;
import com.cqut.edu.basic.Common;
import com.cqut.edu.basic.MoveGen;

public class LocalPlayView extends View {

	private LocalActivity activity;

	public LocalPlayView(Context context, LocalActivity activity) {// 构造器
		super(context);
		mContext = context;
	...
	public boolean onTouchEvent(MotionEvent e) {
		...
		if (!Common.turnMyGo && Common.isgamestart) {
			// AI行棋
			// Thread.sleep(3000);
			int count = MoveGen.GetAllMoves(Common.enemyCMap, 0, 0);
			int index = Math.abs(new Random().nextInt()) % count;
			Move move = MoveGen.m_MoveList[0][index];
			Common.i1 = 11 - move.From.x;
			Common.j1 = 4 - move.From.y;
			i_j[0] = 11 - move.To.x;
			i_j[1] = 4 - move.To.y;
			DealRes(i_j);
			// ShowSomeThing(Common.i1 + " " + Common.j1 + "->" + i_j[0] + " "
			// + i_j[1]);
